...
def test_match():
    ...
    assert match(1).\
                default(1)()                        == Some(1), "default() matches anything"

    assert match(1).\
                equals(2, None)()                   is Nothing, "1 != 2, result is None"

    assert match(2).\
                equals(2, None)()                   is Nothing, "2 == 2, result is None"

    assert match(2).\
                equals(2, lambda inst, _: inst*2)() == Some(4), "2 == 2, result is 2*2"

    assert match(2). \
                equals(2, lambda inst, _: inst * 2).\
                equals(4, lambda inst, _: inst * 3)() == Some(4), "2 == 2, result is 2*2"

    assert match(99). \
                equals(2, lambda inst, _: inst * 2).\
                equals(4, lambda inst, _: inst * 3).\
                default(10)()                         == Some(10), "no match for 99, return default()"

    assert match(7). \
                equals(2, lambda inst, _: inst * 2). \
                equals(4, lambda inst, _: inst * 3). \
                isin([5, 7, 9], lambda inst, _ : inst * 4).\
                default(10)()                        == Some(28), "7 is in [5,7,9], result is 7 * 4"

    assert match(7, 3). \
                equals(2, lambda inst, _: inst * 2). \
                equals(4, lambda inst, _: inst * 3). \
                filter(lambda _ : _ in [5, 7, 9], lambda inst, p: inst * p). \
                default(10)()                        == Some(21), "7 is in [5,7,9], result is 7*3"

    assert match(Foo(1,2)).classof(
        Foo, lambda inst, _ :inst.x+inst.y)()        == Some(3), "Foo instance is of Foo class, return 1+2"

    assert match(Foo(1, 2), 10).classof(
        Foo, lambda inst, p: (inst.x + inst.y) * p)() == Some(30), "Foo instance is of Foo class, return ((1+2)*10"
"""
